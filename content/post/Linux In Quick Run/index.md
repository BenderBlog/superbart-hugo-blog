...
![这个是VMWare虚拟机下，配置共享文件的方式](VMWareFileShare.png)

![这个是VirtualBox虚拟机下，配置共享文件的方式](VirtualBoxFileShare.webp)
...
![这个是VMWare虚拟机的系统快照功能](VMwareSnapshot.jpg)

![这个是VirtualBox虚拟机的系统快照功能](VirtualBoxSnapshot.jpg)
...
![页面上提供了Java(OpenJDK),AOSP(安卓系统源代码)的镜像 下面还有VLC，OBS,Wireshark,FDroid等软件的镜像](TsinghuaMirror.jpg)
...
![SuperBart超级抽象画工时间2333](OldTimeWithTerminal.jpg)
...
![根据维基百科X协议页面画的，不一定准确](XStructure.jpg)
...
![这个小恐龙比那个恶心人的辅导员恐龙好多了，我不理解为啥我们假期那么短](DragonOnMyDesktop.jpg)
...
![看分区的命令是sudo fdisk -l，/boot一个分区，/一个分区，还有16GB的交换空间(虚拟内存)](SDDPartition.jpg)
...
![左面的不显示隐藏文件，右面的显示隐藏文件](HiddenFileComparison.jpg)
...
![他想装一个软件，卡在这里了](ProblemCheck.jpg)
